...
_groupLeader addEventHandler ["GetInMan", {
	...
	_triggerDropOff setTriggerStatements ["this && {isTouchingGround _x} count thisList > 0;", '["dropoffTask","SUCCEEDED"] call BIS_fnc_taskSetState; _fullCrew = fullCrew [(assignedVehicle player), "cargo", false] apply { _x select 0 }; { deleteVehicle _x } forEach _fullCrew; [] call reapercrew_training_fnc_startTraining;', ""];
